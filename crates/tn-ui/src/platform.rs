...
mod imp {
    ...
    /// The Tn application icon (磷光 brand mark: a viewfinder — four phosphor
    /// corner brackets framing a block cursor, in #5BE7C4, on a dark chassis).
    /// Generated by `scripts/gen_icon.py` — 7 sizes (16/24/32/48/64/128/256),
    /// BMP/DIB format (traditional ICO). Embedded in the binary; also embedded as
    /// the exe resource via build.rs.
    ...
    /// (phosphor viewfinder brackets + block cursor on a dark chassis, same as
    /// the taskbar icon). Returns `true` on success.
