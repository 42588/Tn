...
mod imp {
    ...
    /// The Tn application icon (磷光 brand mark: opaque dark chassis + phosphor
    /// `>` prompt + block cursor in #5BE7C4). Generated by `scripts/gen_icon.py`
    /// — 7 sizes (16/24/32/48/64/128/256), BMP/DIB format (traditional ICO).
    /// Embedded in the binary; also embedded as the exe resource via build.rs.
    ...
    /// (phosphor `>` prompt + block cursor on a dark chassis, same as the
    /// taskbar icon). Returns `true` on success.
