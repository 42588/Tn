...
> ③ **agent 活动栏**(`terminal_view::header.rs::render_activity_rail`):agent 正文右栏 = 状态 + 本次改动卡。**✅ 全端口 + 接真实数据**:`.abody` = 正文 + `.arail` 1:1 还原 mockup;**「本次改动」= 真实 `git diff HEAD --numstat`**(`io::spawn_usage_poller` 后台按会话 mtime 刷新、`crate::gitutil` 有界调用 + numstat/preview 解析,**不解析终端正文**,idle agent 零开销);**状态行诚实**——**不伪造「运行中」**(agent 思考/运行态 PTY 不可观测,§见踩坑/产品设计 §6.3),只显 git 摘要(N 文件 + X −Y / 工作区干净);**点改动卡发 `OpenInQuickLook`** → workspace `QuickLook::open_diff` 弹速览 Diff(`.ahint` 故诚实)。配套:正文落 mockup `.body` `11/15` 内边距(`BODY_PAD` 偏移网格/光标/鼠标命中/cols-rows 适配)。
