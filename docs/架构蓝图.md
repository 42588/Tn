...
> - **✅ 冻结修复(`git diff` 不再卡 UI)**:`open()` 原**每次文件树点击 / 预览 `↑↓` 换文件**都同步跑 `git diff`,gpui UI 线程被阻塞 → 大仓库 / `.git` 锁 / 杀软扫描时整窗冻死(真机实测;5-agent workflow + 日志定位:卡在未埋点的预览→Nav→open→git 暗路,零 `SLOW` 告警)。**两层修**:① **惰性 diff**(`diff_dirty`:`open` 不算 diff、只标脏,仅切到 **Diff tab** 时 `ensure_diff` 跑 git → git 离开导航/打开/保存热路径)· ② **有界 + 不闪窗**(`git_capture_bounded`:`.output()` 丢一次性线程 + `mpsc::recv_timeout(1.5s)` 超时即放弃,`CREATE_NO_WINDOW` 防控制台闪)。`explorer::compute_git_status` 同加 `CREATE_NO_WINDOW`。**第二个冻结**(真机日志暴露:git 已离开打开路径,切 HTML 仍冻)病根在 **paint**——`uniform_list` 行闭包 + 文字整形在 gpui paint 阶段跑(不在 `render()` 内),`file_row` 逐 token 拆 `div`、HTML 标记 + **CJK**(代码字体无 CJK → DirectWrite 逐 run 回退)× 几十 span/行 × 可见 ~30 行 → 整形爆量冻窗。修:`coalesce_spans` 按 tint 合并相邻 token + 封顶 `MAX_SPANS` + 长行单 span(`file_row`/`edit_row` 共用),span/整形 run 数大降。诊断埋点(`tn::quicklook` target,`open`/`compute_diff`/预览键/`nav open`/`render START` 计时)暂留,真机确认不再冻后清。
...
> ④ **app 菜单**(点 Tn logo):会话 / 文件·工作区 / 设置 下拉。**✅ 品牌 caret(Tn ▾)已端口**(视觉);⏳ 点击展开的 popup 菜单待端口(logo 改可点 → 窗口拖动挪到标签条空白)。
> ⑤ **explorer 定宽**:**✅ 侧栏改 224px 定宽**(防大窗撑胖)。
> 设计依据见 [产品设计 §2.1 / §6.2 / §6.3 / §6.4](产品设计.md);app 菜单新增 7 个图标待补 `assets.rs`([样式还原手册 §10](样式还原手册.md))。
