...
> ③ **agent 活动栏**(`terminal_view::header.rs::render_activity_rail`):agent 正文右栏 = 当前状态 + 本次改动卡。**✅ 视觉外壳已端口**(`.abody` = 正文 + `.arail`,1:1 还原 mockup,当前为占位示例内容);⏳ 真实数据(`git diff` + 已解析 JSONL,**不解析终端正文**)待接。
...
> **原型同步轨道全部端口完成**(①–⑤ 均 ✅;余下为「真实数据接入」类后续:活动栏 git/JSONL、欢迎最近目录)。设计依据见 [产品设计 §2.1 / §6.2 / §6.3 / §6.4](产品设计.md)。
