...
> - **编辑态 = 自绘小编辑器**(单光标,`buf: Rc<Vec<String>>` + `cursor`):打字插入(读 `key_char`)/ Backspace / Delete / Enter 拆行 / Tab→空格 / 方向键·Home·End·PgUp·Dn 移光标 / `Ctrl+S` 写盘(写后刷新 File+Diff)/ `Esc` 回预览。光标在虚拟化行内按 col 切 [前][caret][后] 渲染;`ROW_H` 固定行高保证 `uniform_list` 一致 + caret 对齐。**缓冲编辑逻辑抽成纯 `op_*` 自由函数,headless 单测**(多字节安全、拆并行、列钳)。
> - ⏳ **后续增量**(非阻塞):选区 + 复制粘贴、撤销/重做、鼠标点位、查找替换;编辑态语法高亮(当前编辑态是纯文本 + caret)。
