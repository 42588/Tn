...
> - **✅ 增量已补全**:**选区**(`Shift`+方向/Home/End/PgUp·Dn,折叠落近/远端)· **复制/剪切/粘贴**(`Ctrl+C/X/V`,无选区时整行;粘贴多行 `op_insert_multiline`,经 gpui clipboard)· **撤销/重做**(`Ctrl+Z` / `Ctrl+Y` / `Ctrl+Shift+Z`,快照栈 + 连续打字合并成一步)· **全选**(`Ctrl+A`)· **鼠标点位**(点击置光标、`Shift`-点击扩选;`char_w` 经 `text_system().advance` 量、code 区 canvas captured bounds 映射 x→列,行号 `i` 已知免滚动偏移)· **编辑态语法高亮 + 选区底色 + caret 三合一**(`edit_row` 按字符展开 tint、按 (tint,选中) 分组、caret 切 run)· **查找/替换**(`Ctrl+F` / `Ctrl+H` 唤条;`Enter`/`Shift+Enter` 下/上个匹配[选中 + 滚入视区]、`Tab` 切查找/替换框、`Ctrl+Enter` 全部替换、`Esc` 关)。**缓冲编辑/选区/查找逻辑全抽成纯 `op_*`/`all_matches`/`selected_text` 等自由函数,headless 单测 14 条**(多字节安全、拆并行、跨行选区、匹配/替换)。
