...
> ① **Quick Look 速览浮层**(`quick_look.rs`,原 `viewer.rs`):文件树选中弹**贴树浮层**(覆盖终端、**不占分屏**),File / Diff 切换 + 行号槽 + 语法/增删着色 + 左缘 accent `seam` 指向选中文件 + `.qlfoot` 键帽提示。**✅ 视觉浮层已端口**(`QuickLook`:`quicklook_fill` 烤实暗玻璃[浮终端上须压住后字]+ `quicklook_frame` 冷能量描边 + 更深浮起投影;点文件树打开、`Ctrl+Shift+J` 收起、Diff/File 点击切换;绝对定位锚到 explorer 右缘[关则锚工作区左缘]);⏳ **键盘两态待接**(`Space` 开 / `↑↓` 换文件实时跟随 / `Enter` 进**编辑态** / 方向键归编辑器 / `Ctrl+S` 保存——需 explorer 键盘焦点 + 可编辑文本缓冲,编辑写盘有风险,按视觉先行轨道延后,同 ③ 活动栏先例)。
