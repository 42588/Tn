...
**🧭 原型同步轨道(Prototype → gpui 端口,⚡ 当前活跃)**:[`design/panels/`](../design/panels/) 下的逐面板原型已**定稿**(共享 [`calm-glass.css`](../design/calm-glass.css);数值见 [样式还原手册 §16](样式还原手册.md);`token_drift` / `roots_mirror` / `no_hardcoded` / `spec_gen` 四道守卫把关),后续工作 = **一步步把原型端口进 gpui**。已锁、待端口:
> ① **Quick Look 速览浮层**(`viewer.rs` → `quick_look`):文件树选中 `Space` 弹**贴树浮层**(覆盖终端、**不占分屏**),File 可编辑 / Diff 只读 / **焦点两态**(预览 `↑↓` 换文件、编辑方向键归编辑器)+ `Ctrl+S`——取代常驻右侧查看器列。
> ② **欢迎 launchpad**(`welcome`):默认新窗/新标签 = 启动磁贴 + 最近目录 + 快捷键提示。
> ③ **agent 活动栏**(`terminal_view::activity_rail`):agent 正文右栏 = 当前状态 + 本次改动卡(数据来自 `git diff` + 已解析 JSONL,**不解析终端正文**)。
> ④ **app 菜单**(点 Tn logo):会话 / 文件·工作区 / 设置 下拉(logo 改可点 → 窗口拖动挪到标签条空白)。
> ⑤ **explorer 定宽 + 比例重排**(侧栏防大窗撑胖、shell 给点高度)。
