...
> ④ **app 菜单**(点 Tn logo):会话 / 文件·工作区 / 设置 下拉。**✅ 全端口**(`workspace::render_app_menu`):品牌可点(去掉品牌的 `WindowControlArea::Drag`,拖窗靠标签条 spacer)→ 弹 `.appmenu` popup(248px、`pane_fill` 暗玻璃 + rim + 深投影 + specular)+ **点击外部 scrim 关闭** + caret 开态。**11 项全接真实动作**:新会话→命令面板 · 新标签 · 打开文件夹→`prompt_for_paths` 选目录后 `explorer::set_root` 重定根树 · 在资源管理器中显示→`cx.reveal_path(cwd)` · 文件浏览器→toggle · 设置→`cx.open_with_system(config_path)` · 主题→`cx.reveal_path(themes_dir)` · 重载配置 · 关于→`open_with_system(README)` · 退出→`cx.quit()`(+`Ctrl+Shift+Q` 绑定)。新增 7 图标(external/sidebar/sliders/moon/refresh/info/power)入 `assets.rs`。
