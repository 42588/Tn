...
| 主题 | 文件 | 用途 |
|---|---|---|
| 宠物系统规则(v2 陪伴版) | [宠物系统规则](宠物系统规则.md) | 已按阅读地图/版本关系/规则分层/事件优先级整理;北极星「每天期待见面」+ v2.2 主动互动记忆 + v2.3 磷光通道空间共演 + 打扰预算与反规则 |
| 交互动画实现方案(可落地 · as-built) | [宠物交互动画实现方案](宠物交互动画实现方案.md) | 每个动画**具体怎么画**:14×12 网格 quad 词汇、帧驱动模型、逐帧 dx/dy/缩放/镜像数值、颜色表、性格参数表;数值与 `pet.rs` 一致(规格即落地参照)|
...
- v2.3 磷光通道是 Tn 的标志性动效语言,只服务幽灵终端、Quick Look、浮层、分屏、欢迎页/工作区和远端重连等关键转场;reduced motion 下必须降级为静态换岗。
