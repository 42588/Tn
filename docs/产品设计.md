...
| app 菜单(点 logo · 会话/文件/设置) | [panels/01](../design/panels/01-window-chrome.html) | [workspace.rs](../crates/tn-ui/src/workspace.rs) `app_menu`(待端口,popup) |
...
| 面板 | 原型 | 实现 |
|---|---|---|
| Agent 面板(可靠头部 + 用量环 + 终端正文 + 右侧活动栏/改动卡) | [panels/02](../design/panels/02-workspace-panes.html) | [terminal_view/header.rs](../crates/tn-ui/src/terminal_view/header.rs) `render_agent_header` + 终端 + `activity_rail`(待端口,git/JSONL) |
