...
| 面板 | 原型 | 实现 |
|---|---|---|
| Agent 面板(可靠头部 + 用量环 + 终端正文 + 右侧活动栏/改动卡) | [panels/02](../design/panels/02-workspace-panes.html) | [terminal_view/header.rs](../crates/tn-ui/src/terminal_view/header.rs) `render_agent_header` + 终端 + `render_activity_rail`(✅ 视觉外壳已端口 `.abody`=正文+`.arail`;数据 git/JSONL 待接) |
