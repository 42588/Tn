...
| 速览编辑 Quick Look(贴树浮层 · 浮于终端) | [panels/03](../design/panels/03-side-panels.html) | [quick_look.rs](../crates/tn-ui/src/quick_look.rs) `QuickLook`(✅ 视觉 + 全套交互已端口:暗玻璃 `quicklook_fill` + 冷能量描边 + 左缘 `seam` + 虚拟化代码区;预览 `↑↓` 换文件实时跟随 / `⇥` 切 Diff·File / `Esc`·`Space` 关;`Enter` 进**自绘小编辑器**:打字/方向键/Tab/Backspace·Delete/`Ctrl+S` 写盘/`Esc` 回预览;⏳ 选区·复制·撤销·鼠标点位为后续增量) |
...
| 文件树 + 速览编辑(Quick Look) | tn-ui | M4 → 端口中 | 文件树 ✅(+键盘 nav);Quick Look 浮层 ✅(视觉 + 预览导航 + 自绘编辑器写盘);选区/撤销/复制/鼠标点位 🧭 后续增量 |
| 每格颜色 + 焦点描边 | tn-ui | M1 | ✅ |
