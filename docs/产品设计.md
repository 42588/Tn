...
| 速览编辑 Quick Look(贴树浮层 · 浮于终端) | [panels/03](../design/panels/03-side-panels.html) | [quick_look.rs](../crates/tn-ui/src/quick_look.rs) `QuickLook`(✅ 视觉浮层已端口:暗玻璃 `quicklook_fill` + 冷能量描边 `quicklook_frame` + 左缘 `seam` + Diff/File 点切;点文件树打开、`Ctrl+Shift+J` 收起;⏳ `Space`/`↑↓`/`Enter` 编辑两态 + `Ctrl+S` 待接) |
...
| 文件树 + 速览编辑(Quick Look) | tn-ui | M4 → 端口中 | 文件树 ✅;Quick Look 浮层 ✅ 视觉已端口(`quick_look.rs`);键盘两态 + 编辑写盘 🧭 待接 |
| 每格颜色 + 焦点描边 | tn-ui | M1 | ✅ |
