...
- **速览浮层(Quick Look)**:`Space` 触发、贴树右缘滑出、浮于终端之上;**↑↓ 在树里换文件,浮层实时跟随**(像 macOS Quick Look 边翻边看)。头部 = 文件图标 + 路径 + 「编辑中」+ **Diff/File 切换**;正文 = 行号槽 + 语法高亮 + 增删着色;底栏提示随模式变(File 给 `Ctrl+S 保存`、Diff 给 `只读 · ⇥ 切 File`,公共 `↑↓ 换文件 · Esc 关闭`)。浮层比常驻面板更实(压住后面终端、保证代码可读)+ 左缘 accent 竖线指回选中文件。
  - **File 模式(可编辑)**:整文件 + 行号 + 语法高亮(`syntect` 或 tree-sitter);**便捷编辑 + `Ctrl+S` 保存——编辑只落此模式**。
  - **Diff 模式(只读审阅)**:某次改动的 diff(红 `-` / 绿 `+`),来源 = agent 工具调用事件(opt-in 桥)/ 文件前后快照 / `git diff`。**"Claude 改了 element.rs" → Space 直接看 diff**,审阅不切窗口;**只读**(在增删行里直接编辑很怪),要改按 `⇥` 切 File。
