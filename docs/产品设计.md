...
- **窗格/浮层背景用纯色、不铺渐变**(2026-06-01 定):大面积渐变在 8-bit 色深会断层色带(mockup 靠噪点 dither + backdrop 模糊抹平,gpui 都不支持)。`pane_fill`/`quicklook_fill` 改为渐变两端的中点纯色(**零色带**);`specular_top` 顶部柔光洗移除;面板投影晕影移除(`shadowed`→no-op)。深度感现仅靠 1px 渐变边框环 + 面板间距 11px 分隔。这是 GPUI 0.2.2 硬约束下的取舍。
...
| 面板 | 原型 | 实现 |
|---|---|---|
| Agent 面板(可靠头部 + 用量环 + 终端正文 + 右侧活动栏/改动卡) | [panels/02](../design/panels/02-workspace-panes.html) | [terminal_view/header.rs](../crates/tn-ui/src/terminal_view/header.rs) `render_agent_header` + 终端 + `render_activity_rail`(✅ `.abody`=正文+`.arail`;**「本次改动」接真实 `git diff HEAD`**[`crate::gitutil`,`notify` 文件监听**变化即刷新**,不解析终端]·**诚实状态行不伪造「运行中」**·点卡片发 `OpenInQuickLook`→速览 diff;**shell 内敲 claude/codex 自动切 agent 态** `sync_shell_agent`) |
