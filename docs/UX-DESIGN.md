...
> **默认主题 = `Tn Dark`**(Tokyo Night 调校):定义见 [`config/themes/tn-dark.toml`](../config/themes/tn-dark.toml);**高保真原型见 [`design/mockup.html`](../design/mockup.html)**(浏览器打开),渲染图 `design/mockup.png`。原型展示了默认 "vibe coding" 布局:**文件树 Explorer | Claude 大屏 + 小 shell | Diff 查看器**,以及上下文环/用量读数、Warp block、活动屏焦点光、半透明窗口质感。
