...
> 本节由 `TN_GEN_SPEC=1 cargo test -p tn-ui spec_gen` 从 `design/mockup.html` + `tn-dark.toml` + `style.rs` 直接生成,
> 与代码/主题保证一致(再由 `style::token_drift` 测试守卫);**全文数值以本节为准**。改了 mockup/主题后重跑刷新。
