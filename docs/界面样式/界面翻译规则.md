...
本文负责承载 `design/mockup.html` / `design/calm-glass.css` 到 GPUI Rust 实现的具体译法，包括 token、布局、颜色、字体、边框、阴影、材质、图标、状态和完整对照示例。母文档只保留入口索引和守卫规则。

## 背景

原内容来自 `docs/界面样式实现规则.md` 的 CSS → gpui 翻译词典部分。每一条仍按 `gpui 0.2.2` 源码或 mockup / 主题原件核实；带 **⚠** 的条目表示 gpui 0.2.2 无法精确还原，带 **✗** 的条目表示曾经写错、已修正。

引用位置（cargo registry 内）：`gpui-0.2.2/src/{styled.rs, color.rs, geometry.rs, text_system.rs, window.rs, elements/div.rs}`、`gpui-macros-0.2.2/src/styles.rs`。
...
> 本节讲**映射关系与写法(HOW)** + alpha 折算原理。**令牌的当前权威数值(WHAT)以 [组件规格与自动生成数值](组件规格与自动生成数值.md) 为准**——那里由工具从 `tn-dark.toml`/`style.rs` 直出、`token_drift` 守卫一致。
...
- **足版**:面板内叠两条 absolute 1px hairline——顶 `linear_gradient` 冷白 `.20`、底 accent `.13`(竖向渐隐),模拟「顶承光 / 底回光」;聚焦态调亮(对照 mockup `.pane.active::before`,`.pane` 值见 [组件规格与自动生成数值](组件规格与自动生成数值.md))。
...
|---------|------------|------|
| `@keyframes pulse`（opacity + scale） | 脉冲指示(透明度) | `opacity` 布尔 toggle + notify（⚠ 无 `transform: scale`，只脉冲透明度；**不伪造 agent Thinking**,见 [智能体用量与诚实原则](../产品体验/智能体用量与诚实原则.md)） |
...
- 状态：已从母文档迁移，语义保持原样。
- 数值类权威表仍以 [组件规格与自动生成数值](组件规格与自动生成数值.md) 为准。
- 视觉与生成守卫见 [视觉守卫和生成流程](视觉守卫和生成流程.md)。
