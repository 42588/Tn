...
5. **统一的 `evict_render_image` 物理释放与多窗口清理**：
    - 每次关闭 QuickLook、切换文件或 View 被 Drop 析构时，遍历所有打开的窗口并对每一处调用 `cx.drop_image(render_image, Some(window))` 清理 DirectX 显存中的 `SpriteAtlas` 缓存。这解决了在多窗口模式下（如存在悬浮 QT 窗口时）只清理第一个窗口导致主窗口的 GPU 纹理无法正确驱逐、显存随切换而增长的问题。
