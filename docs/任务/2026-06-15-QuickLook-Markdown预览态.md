...
## 真机修复:紧凑列表行内内容被吞(2026-06-15)

首版真机截图暴露 bug:`- 文字 \`行内码\`;更多` 这类**紧凑列表(tight list)**项里,
行内码消失、链接文字整段不见、且每个文本碎片各自换行。

- 根因:pulldown-cmark 对紧凑列表**不把项内容包进 `Paragraph`**,而是直接把
  `Text`/`Code`/`Start(Link)` 等**行内事件吐到块级层**。初版 `md_blocks` 只认 `Text`
  (每段当独立段落 → 换行)、没处理 `Code`(丢弃)、`Start(Link)` 落到 `_` 分支被当
  容器递归吃掉(链接文字消失)。
- 修复:重写 `md_blocks` —— 块级层也维护行内样式栈累积 `inl`,处理
  `Text/Code/Strong/Emphasis/Strikethrough/Link/Image/SoftBreak/HardBreak/TaskListMarker`,
  仅在真正的块边界(段落 End / 块容器 / 分隔线)才 flush 成段落;用 `TagEnd` 显式区分
  「行内 End(弹栈)」与「容器 End(收尾退出)」。
- 守卫:新增 `tight_list_item_emits_inline_events_without_paragraph_wrapper` 锁定该事件形状。
...
用户真机确认通过:各 md 元素排版、大文件滚动、Enter↔Esc 来回切与未保存镜像、CJK 正文换行均正常。任务收尾,移入 TODO 已完成。
