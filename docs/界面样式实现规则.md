...
|---------|------------|------|
| `@keyframes pulse`（opacity + scale） | 脉冲指示(透明度) | `opacity` 布尔 toggle + notify（⚠ 无 `transform: scale`，只脉冲透明度；**不伪造 agent Thinking**,见 产品体验索引 §6.3） |
...
- **活动栏 `.arail`**(agent 右栏,[panels/02](../design/panels/02-workspace-panes.html)):agent body 用 `flex().flex_row()`,内放 `终端流(flex_1 + min_w(px(0.)))` + `.arail`(`flex_none().w(px(212.))` + 左 1px 分隔)。栏内 `.astat` 状态行 + `.achip` 改动卡(`.afile` 文件名 / `+N −N` + `.adiff` 行;当前文件 `.cur` 带 accent 内 ring)。**数据来自 `git diff` + JSONL,不解析终端正文**(产品体验索引 §6.3)。
- **速览浮层 `.quicklook`**([panels/03](../design/panels/03-side-panels.html)):贴树右缘的 absolute 浮层(层级高于终端),bg 比常驻面板更实(~0.9 alpha 压住后面终端);头 `.vh` + 正文 `.code`(行号 `.ln` / 增删 `.cl.add/.del`)+ 底 `.qlfoot`;左缘 `.seam` accent 竖线。真机宜独立 popup/overlay 层。**两态键位**:预览态 ↑↓ 换文件、编辑态方向键归编辑器(产品体验索引 §2.1)。
- **app 菜单 `.appmenu`**([panels/01](../design/panels/01-window-chrome.html)):logo 下 absolute 下拉,`.mi` 项(`hover` 高亮、`.danger` 红)+ `.sep` 分隔;`.brand .caret` 提示可点。logo 改可点后**拖窗挪标签条空白**(产品体验索引 §6.2)。
