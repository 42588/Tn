...
## 图形界面 / 样式开发(**必走此路**)
任何 UI / 颜值改动都沿这条链走,别绕:
1. **设计真源 = [`design/mockup.html`](design/mockup.html)**(浏览器打开即标准)。要改样子,先改/看 mockup。**逐面板原型** = [`design/panels/`](design/panels/)(共享 [`design/calm-glass.css`](design/calm-glass.css),入口 `index.html`);**面板 ↔ Rust 还原 ↔ 真实实现 分类清单** = [`docs/UI-CATALOG.md`](docs/UI-CATALOG.md)。
2. **查确切值 → [`docs/CSS_TO_GPUI.md`](docs/CSS_TO_GPUI.md)**:§1–§15 是 CSS→gpui 译法(**HOW**),**§16(自动生成)是每个组件的权威数值(**WHAT**)——照抄不估**。改了 mockup/主题后跑 `TN_GEN_SPEC=1 cargo test -p tn-ui spec_gen` 刷新 §16。
...
5. **真机肉眼验 = [`docs/test/mockup_replica.rs`](docs/test/mockup_replica.rs)**:`cargo run -p tn-ui --example mockup_replica` 弹出 mockup 的 gpui 复刻,与真实 app(`cargo run -p tn-app`)/ mockup 并排比。
...
cargo test  --workspace --lib                  # 120 单测(tn-core 25 / tn-config 27 / tn-ui 29 / tn-shell 11 / tn-blocks 5 / tn-ai 15 / tn-pty 8)+ `--test conpty_pipeline` 1 集成
#   注:`--workspace`(无 --lib)会编译 docs/test/mockup_replica.rs 这个 owner 的设计复刻 example;它若 WIP 不编译会挡住测试,用 --lib 只跑库单测绕开
