...
- **里程碑 M0–M5 全部落地**(owner 执行顺序 `M0 → M1 → M3 → M4 → M5 → M2`)。可日用:本地 pwsh/cmd、
  ...
  Calm Glass 颜值。单测共 **134** + 1 集成测试。变更见 [CHANGELOG.md](CHANGELOG.md),路线图见 [docs/架构蓝图.md](docs/架构蓝图.md) §8。
...
cargo test  --workspace --lib                  # 库单测(tn-core 25 / tn-config 27 / tn-ui 46 / tn-shell 11 / tn-blocks 5 / tn-ai 15 / tn-pty 8)+ `--test conpty_pipeline` 1 集成
...
- **`uniform_list` 的行闭包 + 文字整形在 gpui 的 *paint* 阶段跑,不在你的 `render()` 里——所以"逐 token 一个 `div`"的代价 `render` 计时器看不见、却能冻死整窗**:Quick Look 切到 HTML(密集标记 + **CJK**)时整窗冻、`render()` 却无 `SLOW`。病根:`file_row` 把一行 `highlight()` 的每个 token 拆成独立 `div`(几十/行),paint 时 gpui 要对**每个 span 单独 layout + 整形**;代码字体无 CJK → DirectWrite **逐 run 字体回退**(慢),× 每行几十 span × 可见 ~30 行 → paint 爆量。诊断:`render()` 很快返回(无 `render SLOW`)、日志停在 paint 前;**在 `render` 开头打 `render START` debug**——UI 冻的几秒里 appender 线程仍能刷出它,故它是冻死前最后一行 = 坐实冻在 paint(`open` 已返回)。**修**:`coalesce_spans` 按 tint 合并相邻 token(标记行 ~30 span → 个位数)+ 封顶 `MAX_SPANS(48)`(尾部折叠、不丢内容)+ 长行(>2000B)整行单 span;span/整形 run 数大降 → paint 廉价。**通则:虚拟化只省了"行数",一行内的 span 数同样要控**(尤其 CJK / 长行)。Ctrl+C 杀进程会丢 `non_blocking` appender 未刷的缓冲,但**多秒 UI 冻**期间 appender 线程有时间刷出冻前已发送的行(故 `render START` 仍可见)。
