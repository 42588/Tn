...
- **定位 GPU paint 期的冻结/崩溃:加"开机即跑"探针让自己能复现,别只靠用户**:`uniform_list` 行闭包 + 文字整形在 gpui *paint* 跑(`render()` 返回之后),`render()` 计时器照不到,且 `non_blocking` appender 的缓冲在 Ctrl+C 杀进程时丢失 → 日志"暗"。`TN_QL_BENCH=<file>` 探针:开机在真窗口里把该文件弹进 Quick Look、2.5s 后自退——paint 挂则进程挂(命令超时即复现)、paint 崩则进程崩(`cargo run` 直接拿到 `0xc0000409` + `RUST_BACKTRACE` 顶帧 `rust_oom ← grow_one ← highlight:146` 直指死循环)。比反复让用户复现快得多;探针 env 门控、保留备用(同 `TN_RESIZE_EXP` 取向)。诊断技巧:UI 冻**多秒**期间 appender 线程仍能刷出冻前已发送的行,故"在可疑段开头打一条 debug"能把最后一行钉在罪段。**paint 成本顺带加固**(非此次根因):`coalesce_spans` 按 tint 合并相邻 token + 封顶 `MAX_SPANS(48)` + 长行整行单 span,降密集行的 div/整形数。
