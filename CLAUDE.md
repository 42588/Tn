...
tn-pty     PtyBackend trait + LocalPty(ConPTY)+ wsl.rs(M2:parse_distros 解码 wsl --list --quiet
           的 UTF-16LE + list_distros)+ ssh.rs(M2b:SshBackend 实现 PtyBackend——专属 tokio 线程把
           russh 的 async channel 桥成同步 Read/Write + SshConfig)。headless。(WSL ✅ 复用 LocalPty 跑
           wsl.exe;SSH 编译+单测过、端到端待真机)
...
cargo test  --workspace                        # 单测共 93(tn-core 10 / tn-config 26 / tn-ui 18 / tn-shell 11 / tn-blocks 5 / tn-ai 15 / tn-pty 8)
...
## 后续里程碑
执行顺序(owner 调整后):**M3 → M4 → M5 → M2**。M3 shell 集成 + block(✅)、M4 Claude/Codex 托管 + 用量 + 命令面板(✅ 功能闭环)、M5 Quick Terminal(✅ headless 闭环,待真机肉眼),**M2:WSL ✅ 完成**(复用 LocalPty 跑 wsl.exe + wsl.rs 枚举;命令面板/Quick Terminal 启动器**自动发现所有已装发行版** `discover_profiles` —— config profiles + `wsl --list --quiet`,去重 + 滤掉 docker-desktop*)。**SSH = owner 决定暂停(parked)**:已落地编译+headless 单测过(russh SshBackend[ring 后端] + TerminalView 抽象到 Box<dyn PtyBackend> + SSH profile 接线),代码原地保留;**端到端 + ssh-agent/known_hosts/密码交互/重连待 owner 有远程登录需求时再继续**,在此之前别主动推进 SSH。文件/Diff 查看器 = M3/M4(`PaneContent = Session | Viewer`)。详见 [docs/BLUEPRINT.md](docs/BLUEPRINT.md) §8。
...
- **聚焦 `track_focus` 浮层要在 `render` 里、别在动作回调里**:在动作(如 `toggle_palette`)里 `handle.focus(window)` 时,浮层元素**本帧还没渲染**,焦点落不上 → 键漏给底层焦点元素(实测:命令面板 ↑↓/Enter/Esc 全跑到底层终端)。解法:置一个 `*_needs_focus` 标志,在 `render` 里(浮层的 `track_focus` 元素此帧已在树中)`handle.focus(window)`。Quick Terminal 启动器本就在 render 聚焦(`pending_focus`),所以一直正常——同一套。
...
- **russh(M2b)默认 crypto 后端 `aws-lc-rs` 在本机编译失败**:`aws-lc-sys` 要 **NASM** + 一个能过 cl.exe `stdalign_check.c` 探测的工具链,本机没装 NASM → build script panic。解法:`russh = { default-features = false, features = ["ring", "flate2", "rsa"] }` 换 **`ring`** 后端(自带预生成汇编,无需 NASM)。`rsa` 特性供 `best_supported_rsa_hash`/RSA 公钥。
- **russh 是 async,PtyBackend 是同步**:SshBackend 用专属线程跑 current-thread tokio + `select!` 循环,把 `ChannelMsg::Data` 经 `std::mpsc` 喂同步 reader(recv 阻塞=自然 EOF)、同步 writer 推 tokio channel→`channel.data_bytes`。`select!` 里只让 `channel.wait()`(&mut)出现在分支表达式;`data_bytes`/`window_change`(&self)放分支体里(wait 的借用此时已释放)——照搬 russh 官方 example 的借用模式,否则 `channel` 同时 &mut+& 冲突。
