...
cargo test  --workspace                        # 单测共 71(tn-core 10 / tn-config 14 / tn-ui 16 / tn-shell 11 / tn-blocks 5 / tn-ai 15)
...
- ✅ **Calm Glass 颜值落地**(`lib.rs` + `workspace.rs` + `block_view.rs`):窗口按主题 `[ui.window].backdrop` 设 `WindowBackgroundAppearance::Blurred`(Windows acrylic 模糊背景);chrome 用 alpha 半透玻璃(`cola()` + 令牌 RIM/SHEEN/INSET/HOVER)透出材质,圆角(窗口 16 / 面板 14 / 卡片 11)、玻璃边(rim,替代硬描边)、顶部镜面高光(sheen)、柔和投影(`soft_shadow`,gpui 0.2.2 无 `.shadow_*` → 经 `style().box_shadow`);焦点 pane 暖色细描边 + 浮起、标签 agent 身份点 + 玻璃 pill、命令面板浮层。**无发光**。
...
- 🧭 **剩(肉眼/真机)**:窗口内颜值微调 + 真机 Codex 用量复核 + 标题栏拖动/控制按钮真机点验 + 连续动画(运行/Thinking——需帧时钟,且 agent 思考态 PTY 不可观测,故未伪造)。键位:`Ctrl+Shift+B` 浏览器、`Ctrl+Shift+J` 查看器、`Ctrl+Shift+P` 命令面板。
