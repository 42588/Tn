...
- **里程碑 M0–M5 全部落地**(owner 执行顺序 `M0 → M1 → M3 → M4 → M5 → M2`)。可日用:本地 pwsh/cmd、
  ...
  Calm Glass 颜值。单测共 **119**。变更见 [CHANGELOG.md](CHANGELOG.md),路线图见 [docs/BLUEPRINT.md](docs/BLUEPRINT.md) §8。
...
cargo test  --workspace                        # 单测共 119(tn-core 22 / tn-config 26 / tn-ui 32 / tn-shell 11 / tn-blocks 5 / tn-ai 15 / tn-pty 8)
