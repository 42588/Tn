...
tn-ui      GPUI 前端(唯一链接 gpui 的库):style(共享 Calm Glass 令牌 + col/cola/soft_shadow/shadowed/icon,
           ...
           explorer(文件树侧栏)· welcome(欢迎 launchpad:新标签/首标签的启动磁贴 + 快捷键,点磁贴启动会话)· viewer(文件/Diff 查看器 → 重设计为 quick_look 速览浮层,待端口)· workspace(标题栏 + 标签/n-ary 分屏 + 侧栏 +
...
- **Calm Glass UI**(`assets`/`lib`/`workspace`/`explorer`/`viewer`):SVG 线性图标 + 动态用量环 · 自绘集成标题栏
  ...
  着色 + `git diff`;侧栏带 `✕` 鼠标关闭——**设计已改 `Space` 速览浮层 Quick Look,待端口**,见 [架构蓝图 §8](docs/架构蓝图.md))· 多段状态栏 · 玻璃材质(默认 `Opaque`、圆角 16/14/11、rim/sheen、
  `box_shadow` 柔影,**无发光**)。
