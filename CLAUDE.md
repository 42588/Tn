...
**gpui 焦点 / keymap(真机证实)**
- **中文 / IME 输入必须接 `EntityInputHandler`,否则全 app 打不出中文(只有 ASCII)**:gpui 在 Windows 上 WM_KEYDOWN→`key_down`,**IME 合成文本(拼音→中文)只经 `EntityInputHandler` 投递**(WM_IME_COMPOSITION → `replace_and_mark_text_in_range`/`replace_text_in_range`)。没接处理器时只有 `key_down.keystroke.key_char`(ASCII)能用,中文永远丢——这就是「终端 + 编辑器都无法输中文」的根因。**接法**:① 给 view 实现 `EntityInputHandler`;② 在 **paint 阶段**(`canvas` 的 paint 闭包里)`window.handle_input(&focus_handle, ElementInputHandler::new(bounds, cx.entity()), cx)` 注册(只在聚焦时生效;`handle_input` 有 `debug_assert_paint`,不能在 render body 直接调);③ `marked_text_range` 返回 `Some` ⇒ gpui 知道在合成、把键喂给 IME(见 events.rs:384 `is_composing` 早退)。**关键去重坑**:gpui 的 keydown 处理里,**事件未被 handled(未 `stop_propagation`)才会 `translate_message` 生成 WM_CHAR** → 喂给 `replace_text_in_range`。所以 `on_key` **对已处理的键必须 `cx.stop_propagation()`**,否则接了处理器后每个 ASCII 键 = on_key 写一次 + WM_CHAR 又写一次 = **双输入**。终端/编辑器都靠「英文走 on_key+stop、中文走输入处理器」隔离;终端无可编辑文档 → IME 文本模型只取「合成中的 preedit」、提交直写 PTY。
