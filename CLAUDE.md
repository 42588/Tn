...
> **现状(2026-05)**:全 UI 已**逐组件严格对齐 mockup**(tab / explorer / 查看器 / 各面板 g1 玻璃 / 终端正文落玻璃 / agent 头 / shell 头 / 状态栏 / block 卡 / 标题栏),**主题色硬编码清零**。新组件照此链做,**四道守卫**自动把关。**新一轮原型(Quick Look 速览浮层 / 欢迎 launchpad / agent 活动栏 / app 菜单)已在 [`design/panels/`](design/panels/) 定稿、待端口**——后续围绕「原型 → gpui 同步」推进,见 [产品设计 §6.4](docs/产品设计.md) + [架构蓝图 §8 原型同步轨道](docs/架构蓝图.md)。
