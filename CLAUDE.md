...
tn-ui      GPUI 前端(唯一链接 gpui 的库):style(共享 Calm Glass 令牌 + col/cola/soft_shadow/shadowed/icon,
           ...
           terminal_view/(文件夹模块:mod 渲染核心[正文落 `.body` 11/15 内边距:`BODY_PAD` 偏移网格/光标/鼠标命中/适配]+ io 后台线程[用量 + **活动栏 git** 轮询]+ header agent 头/活动栏[`rail_files` 真实 `git diff HEAD`、诚实状态、点卡片发 `OpenInQuickLook`]+ launch LaunchSpec;可见光标 + generation 渲染缓存)· perf(PerfStats,TN_PERF 渲染日志)· block_view(block 卡片)·
