...
- **里程碑 M0–M5 全部落地**(owner 执行顺序 `M0 → M1 → M3 → M4 → M5 → M2`)。可日用:本地 pwsh/cmd、
  ...
  Calm Glass 颜值。单测共 **127** + 1 集成测试。变更见 [CHANGELOG.md](CHANGELOG.md),路线图见 [docs/架构蓝图.md](docs/架构蓝图.md) §8。
...
cargo test  --workspace --lib                  # 库单测(tn-core 25 / tn-config 27 / tn-ui 39 / tn-shell 11 / tn-blocks 5 / tn-ai 15 / tn-pty 8)+ `--test conpty_pipeline` 1 集成
...
- **Quick Look 编辑器增量**:当前是单光标自绘小编辑器(增删/移动/`Ctrl+S`)。后续:选区 + 复制粘贴 + 撤销/重做 + 鼠标点位 + 查找替换 + 编辑态语法高亮(现编辑态纯文本 + caret)。
