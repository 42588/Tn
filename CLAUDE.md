...
- **当前在做 = M3 + M4**(owner 调整计划:**M3 shell 集成 + M4 AI 托管/用量先行,M2 WSL/SSH 后置**——M3/M4 作用于本地终端,不依赖 M2)。**M3 头部基础已就绪**:`tn-shell`(OSC 旁路解析)+ `tn-blocks`(block 状态机),均 headless + 单测;剩余 M3 = 接线进 `tn-ui` + `block_view` UI。详见下方「## M3 + M4 计划与进度」。
...
铁律:`gpui` 只能出现在 `tn-ui`/`tn-app`。`tn-core`/`tn-pty`/`tn-config`/`tn-shell`/`tn-blocks` 必须能 headless 编译与测试。别把 alacritty 的类型泄漏出 `tn-core`(例如 `CellRun` 暴露 bold/italic 布尔,而非 `Flags`);`tn-shell` 用 `vte` crate 直连(不经 alacritty)。

## 构建 / 运行 / 测试
`cargo` 在 `%USERPROFILE%\.cargo\bin\cargo.exe`(不在 bash PATH 上——PowerShell 里用全路径,或在新开的 shell 里它在 PATH 上)。
```powershell
cargo build --workspace
cargo test  --workspace                        # 单测共 50(tn-core 9 / tn-config 14 / tn-ui 13 / tn-shell 9 / tn-blocks 5)
...
- ✅ `tn-shell`(`crates/tn-shell`):旁路 `vte::Parser` 只处理 `osc_dispatch`,识别 OSC 133(FTCS `A/B/C/D[;exit]`)、OSC 633(+`E` 命令行、`P;Cwd=`)、OSC 7(`file://`→cwd,含 `%XX` 解码与 Windows 盘符)→ `BlockEvent`。`Integration`:per-session nonce + pwsh 集成脚本(prompt 钩子发 D/A/B,PSReadLine Enter 发 C;**脚本为草稿,待真实 pwsh 调**)。9 测试。
- ✅ `tn-blocks`(`crates/tn-blocks`):`BlockModel` 状态机 `Prompt→Input→Running→Finished`;`on_event(ev, line, at_ms)` 聚合成 `Block{command,cwd,prompt_line,output_start/end,exit,started/finished_at}`;中断块(无 `D`)在新 prompt 隐式收尾;`duration_ms`/`succeeded`/`is_running`。5 测试。
- 🧭 **接线**:`TerminalView` 启动注入 pwsh 脚本;reader 喂 Term 的同时跑 `ShellParser` → 用当前光标绝对行 + 时间喂 `BlockModel`(autoquit 验不回归)。
- 🧭 **`tn-ui::block_view`**:Warp 式 block chrome(状态条 成功绿/失败红/运行蓝、命令、时长、cwd),折叠/复制/重跑;**alt-screen 进入即关 chrome**(正确性门槛);锚行随 reflow 重解析。

**M4 — 托管 Claude/Codex + 用量 + 命令面板**(均后于 M3)
- 🧭 `tn-ai::detect`(启动意图 > 进程树 > OSC 标题 > banner);一次 agent 会话当一个 SurfaceBlock,只装饰外框(名称/状态/耗时/cwd)。
- 🧭 `UsageProvider`(headless,可单测):解析 `~/.claude/projects/**/*.jsonl` 与 Codex `$CODEX_HOME/sessions/**/rollout-*.jsonl` → 上下文占用 + token + 估算花费;展示为分屏头环形读数 + AI 状态栏 + 用量面板(见 [docs/UX-DESIGN.md](docs/UX-DESIGN.md) §5)。
- 🧭 命令面板 `Ctrl+Shift+P`:一键起 Claude/Codex(数据源 = config `[[profiles]]`)、新建 pwsh/WSL/SSH、最近命令、block 动作。
...
## 后续里程碑
执行顺序(owner 调整后):**M3 → M4 → M2**。即 M3 shell 集成 + block(进行中)、M4 Claude/Codex 托管 + AI 用量 + 命令面板,然后回头做 M2 WSL+SSH;M5 Quick Terminal 可任意时机插入。文件/Diff 查看器 = M3/M4(`PaneContent = Session | Viewer`)。详见 [docs/BLUEPRINT.md](docs/BLUEPRINT.md) §8。
