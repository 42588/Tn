...
- **当前在做 = M3 + M4**(owner 调整计划:**M3 shell 集成 + M4 AI 托管/用量先行,M2 WSL/SSH 后置**——M3/M4 作用于本地终端,不依赖 M2)。**M3 已完成**:`tn-shell`(OSC 旁路解析)+ `tn-blocks`(block 状态机)+ 接线(`-EncodedCommand` 注入 pwsh 脚本、reader 旁路喂 `BlockModel`)+ `tn-ui::block_view`(Warp 式 block 底栏,Calm Glass、alt-screen 隐藏);`TN_AUTOQUIT` 验不回归,单测 53。**下一步 = M4**(AI 托管/用量/命令面板)。详见下方「## M3 + M4 计划与进度」。
...
cargo test  --workspace                        # 单测共 53(tn-core 10 / tn-config 14 / tn-ui 13 / tn-shell 11 / tn-blocks 5)
