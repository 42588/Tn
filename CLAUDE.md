...
> **现状(2026-05)**:全 UI 已**逐组件严格对齐 mockup**(tab / explorer / 查看器 / 各面板 g1 玻璃 / 终端正文落玻璃 / agent 头 / shell 头 / 状态栏 / block 卡 / 标题栏),**主题色硬编码清零**。新组件照此链做,**四道守卫**自动把关。**新一轮原型已在 [`design/panels/`](design/panels/) 定稿,正逐项端口**:**主界面已 1:1 复刻**——agent 活动栏(`.abody`=正文+`.arail`,`render_activity_rail`,**视觉外壳完成、占位示例内容、真实 git/JSONL 数据待接**)· 品牌 caret `Tn ▾` · explorer 224px 定宽干净面板(去掉「资源管理器」外栏) · **欢迎 launchpad**(`welcome.rs`:新标签/首标签开启动磁贴 + 快捷键,点磁贴启动会话;`Tab` 加 welcome 态,dummy id 用 `PaneId::MAX` 防与真实 pane 撞、否则关欢迎页会误删 pane 0 致 abort,见坑;最近目录待接)· **Quick Look 速览浮层**(`quick_look.rs`:点文件树/树焦点 `Space` 弹**贴树右缘、浮于终端、不占分屏**的玻璃浮层,`quicklook_fill` 烤实暗玻璃[浮终端上须压住后字]+ `quicklook_frame` 冷能量边 + 左缘 accent seam + **虚拟化代码区**[`uniform_list`,大文件不卡];**全套交互完成**——预览 `↑↓` 换文件实时跟随 / `⇥` 切 Diff·File / `Esc`·`Space` 关,`Enter` 进**自绘小编辑器**[打字/方向键/Tab/`Ctrl+S` 写盘 + **选区(Shift+方向)/ 复制粘贴(Ctrl+C/X/V)/ 撤销重做(Ctrl+Z·Y·Shift+Z)/ 全选(Ctrl+A)/ 鼠标点位(点击置光标·Shift-点扩选)/ 编辑态语法高亮+选区底色+caret 三合一 / 查找替换(Ctrl+F·Ctrl+H,Enter 上下个匹配·Ctrl+Enter 全部替换)**;缓冲/选区/查找逻辑 `op_*`/`all_matches` 纯函数 headless 单测])· **app 菜单 popup**(`render_app_menu`:点品牌弹下拉,品牌去 Drag 改可点[拖窗靠标签条 spacer]、点外 scrim 关、caret 开态;补 7 图标。各项接真实动作:**新会话**`⌃⇧N`=分屏启动器[`render_split_launcher`:选方向→选启动器→分屏起会话]· **新标签**`⌃⇧T`=欢迎页 · **打开文件夹**=`prompt_for_paths`→`explorer::set_root` 重定根 + `cd_shells_to` 把纯 shell 窗格 cd 进去 · **布局**=`render_layout_manager` 7 槽存/取/删 · **文件浏览器**`⌃⇧B` · **设置**=在 Quick Look 编辑器开 `config.toml` · **主题**=暂单选 · **重载配置**=还原默认[panic button,覆盖文件,**与非破坏性 `⌃⇧R` 热重载区分**]· **关于**=open README · **退出**`⌃⇧Q`)。**原型同步轨道(①–⑤)全部端口完成**,余下仅「真实数据接入」类(活动栏 git/JSONL、欢迎最近目录)。后续见 [产品设计 §6.4](docs/产品设计.md) + [架构蓝图 §8 原型同步轨道](docs/架构蓝图.md)。
