...
- **`track_focus` 的「点击即聚焦」会 `prevent_default` → 吞掉标题栏拖拽:焦点锚点放 `body`、别放窗口根**:为让 `Ctrl+Shift+P` 在焦点漂移(点到空 chrome、焦点掉出窗格)时仍可派发,需要一个常驻、落在 `key_context("Workspace")` 下的可聚焦锚点。**别把 `track_focus` 加在铺满全窗的根 div 上**——gpui 给每个 `track_focus` 元素注册「点击即聚焦」监听器,命中时调 `window.prevent_default()`(div.rs:2025);而 Windows 拖窗经 `window_control_area(Drag)` 走 NC 命中(HTCAPTION),gpui 在 `handle_nc_mouse_down_msg` **先把这次按下派发进元素树**,`handled = !propagate || default_prevented` 为真就 `return Some(0)`、**系统拖窗(DefWindowProc)永不触发**。根的聚焦监听器铺满含标题栏拖拽条 → 一按就 `prevent_default` → **整窗静默拖不动**(无日志;`track_focus` 在窗格等小元素上无此问题,只有全窗根撞上拖拽区才坏 → 这次才暴露)。**修法**:`key_context` + `on_action` **留根**(它们不注册鼠标监听、不碰拖拽,且照样覆盖浮层);`track_focus(workspace_focus)` 移到**标题栏下方的 `body`**——它的点击聚焦区盖不到拖拽条。`render` 里泊车焦点到 body 时**别抢 explorer 的焦点**(它也在 Workspace 上下文下,自己保活快捷键;`!explorer_focused && !workspace_focus.is_focused` 才泊)。
