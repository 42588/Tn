...
- **里程碑 M0–M5 全部落地**(owner 执行顺序 `M0 → M1 → M3 → M4 → M5 → M2`)。可日用:本地 pwsh/cmd、
  ...
  Calm Glass 颜值。单测共 **117** + 1 集成测试。变更见 [CHANGELOG.md](CHANGELOG.md),路线图见 [docs/BLUEPRINT.md](docs/BLUEPRINT.md) §8。
...
cargo test  --workspace                        # 117 单测 + 1 集成测试(tn-core 23 / tn-config 26 / tn-ui 29 / tn-shell 11 / tn-blocks 5 / tn-ai 15 / tn-pty 8 / tn-cli conpty_pipeline 1)
...
- **终端内核 + 渲染**(`tn-core` + `terminal_view`):每格 fg/bg 颜色(`row_runs()` run 批处理成样式盒,div 渲染)·
  ...
  resize 联动(按 pane 自身 canvas bounds 算行列;ConPTY 精确跟随 alacritty——曾试行锁定防 resize 吃历史,因 ConPTY≠alacritty 致高频空白已撤销,见坑)·
...
- **分屏交互**:✅ 分隔线鼠标拖拽(**commit-on-release**:拖动只移动一条 2px 预览线、释放才改权重 resize 一次——避免 ConPTY 每帧 resize 抖动 + 把"拖大丢旧历史"降到一次;把手平时隐形、hover 微亮)· 🧭 拖拽停靠(drag-dock:拖到边=分屏、拖到中=标签组)。
...
- **ConPTY 行增高的 resize-repaint 既吃滚动历史、又顶飞提示符 → 但"行锁定"修法已撤销,改回 ConPTY 精确跟随 alacritty**。事实(仍成立):拖分隔线把 pane 拉大时 alacritty 把 `delta` 行从滚动历史**拉进可视区顶部**(reflow 单独不丢——见 `tn-core::resize_preserves_content_via_scrollback`),紧接着 ConPTY 的 resize-repaint 用空白/陈旧缓冲覆盖这些行 → 丢 `delta` 行(`TN_RESIZE_EXP=1` 实测:12→24 丢 LINE_18–29);且**任何 ConPTY 行增高都会把光标/内容重定位、把提示符顶出可视区留大片空白**(`TN_RESIZE_EXP=prompt TN_SPAWN_ROWS=34` 实测 34→120 后 fresh prompt 全空白)。
  ...
  - **现行(简单稳健):ConPTY 行/列始终精确跟随 alacritty**(`render` 里 `new_size != self.size` 时同时 resize 两者)。代价:拖分隔线**拉大**仍可能丢几行**旧**滚动历史(可见的当前内容不受影响),已由 **commit-on-release**(释放才 resize 一次)把影响降到最小。**别再引入 ConPTY≠alacritty 的行锁定**——任何尺寸不匹配都会复活空白。`TN_RESIZE_EXP=prompt`(tn-cli)探针留作回归证据。
