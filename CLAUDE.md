...
tn-pty     PtyBackend trait + LocalPty(ConPTY)+ wsl.rs(M2:parse_distros 解码 wsl --list --quiet
           的 UTF-16LE + list_distros)。headless。(WSL ✅ 复用 LocalPty 跑 wsl.exe;SSH = M2b russh)
...
cargo test  --workspace                        # 单测共 88(tn-core 10 / tn-config 26 / tn-ui 18 / tn-shell 11 / tn-blocks 5 / tn-ai 15 / tn-pty 3)
...
## 后续里程碑
执行顺序(owner 调整后):**M3 → M4 → M5 → M2**。M3 shell 集成 + block(✅)、M4 Claude/Codex 托管 + 用量 + 命令面板(✅ 功能闭环)、M5 Quick Terminal(✅ headless 闭环,待真机肉眼),**M2 进行中:WSL ✅ 端到端验证通过(复用 LocalPty 跑 wsl.exe + wsl.rs 枚举),SSH 🚧 待写 russh 逻辑 + 单测、端到端自验**。文件/Diff 查看器 = M3/M4(`PaneContent = Session | Viewer`)。详见 [docs/BLUEPRINT.md](docs/BLUEPRINT.md) §8。
