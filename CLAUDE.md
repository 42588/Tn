...
## 后续里程碑
执行顺序(owner 调整后):**M3 → M4 → M5 → M2**。M3 shell 集成 + block(✅)、M4 Claude/Codex 托管 + 用量 + 命令面板(✅ 功能闭环)、M5 Quick Terminal(✅ headless 闭环,待真机肉眼),**M2 进行中:WSL ✅ 端到端验证通过(复用 LocalPty 跑 wsl.exe + wsl.rs 枚举);SSH ✅ 编译+headless 单测过(russh SshBackend + TerminalView 抽象到 Box<dyn PtyBackend> + SSH profile 接线),🚧 端到端待真机 + ssh-agent/known_hosts 待接入**。文件/Diff 查看器 = M3/M4(`PaneContent = Session | Viewer`)。详见 [docs/BLUEPRINT.md](docs/BLUEPRINT.md) §8。
