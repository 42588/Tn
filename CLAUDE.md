...
- **里程碑 M0–M5 全部落地**(owner 执行顺序 `M0 → M1 → M3 → M4 → M5 → M2`)。可日用:本地 pwsh/cmd、
  ...
  Calm Glass 颜值。单测共 **121** + 1 集成测试。变更见 [CHANGELOG.md](CHANGELOG.md),路线图见 [docs/架构蓝图.md](docs/架构蓝图.md) §8。
...
tn-ui      GPUI 前端(唯一链接 gpui 的库):style(共享 Calm Glass 令牌 + col/cola/soft_shadow/shadowed/icon,
           ...
           explorer(文件树侧栏)· welcome(欢迎 launchpad:新标签/首标签的启动磁贴 + 快捷键,点磁贴启动会话)· quick_look(原 viewer;文件/Diff 速览浮层:贴树右缘、浮于终端、不占分屏,`quicklook_fill` 烤实暗玻璃 + `quicklook_frame` 冷能量边 + 左缘 seam;视觉已端口,键盘两态/编辑 ⏳)· workspace(标题栏 + 标签/n-ary 分屏 + 侧栏 +
...
cargo test  --workspace --lib                  # 库单测(tn-core 25 / tn-config 27 / tn-ui 33 / tn-shell 11 / tn-blocks 5 / tn-ai 15 / tn-pty 8)+ `--test conpty_pipeline` 1 集成
...
- **Calm Glass UI**(`assets`/`lib`/`workspace`/`explorer`/`quick_look`):SVG 线性图标 + 动态用量环 · 自绘集成标题栏
  ...
  shell cwd)· **文件浏览器侧栏**(`Ctrl+Shift+B`,git M/U 标记)· **Quick Look 速览浮层**(点文件树弹**贴树右缘、浮于终端、不占分屏**的玻璃浮层:行号 + 语法着色 + `git diff` + Diff/File 点切 + 左缘 accent seam;`Ctrl+Shift+J` 收起;键盘两态/编辑 ⏳,见 [架构蓝图 §8](docs/架构蓝图.md))· 多段状态栏 · 玻璃材质(默认 `Opaque`、圆角 16/14/11、rim/sheen、
  `box_shadow` 柔影,**无发光**)。
