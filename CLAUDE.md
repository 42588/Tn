...
> **现状(2026-05)**:全 UI 已**逐组件严格对齐 mockup**(tab / explorer / 查看器 / 各面板 g1 玻璃 / 终端正文落玻璃 / agent 头 / shell 头 / 状态栏 / block 卡 / 标题栏),**主题色硬编码清零**。新组件照此链做,**四道守卫**自动把关。**新一轮原型已在 [`design/panels/`](design/panels/) 定稿,正逐项端口**:**主界面已 1:1 复刻**——agent 活动栏(`.abody`=正文+`.arail`,`render_activity_rail`,**视觉外壳完成、占位示例内容、真实 git/JSONL 数据待接**)· 品牌 caret `Tn ▾` · explorer 224px 定宽干净面板(去掉「资源管理器」外栏);**仍待端口**:Quick Look 速览浮层 / 欢迎 launchpad / app 菜单 popup。后续见 [产品设计 §6.4](docs/产品设计.md) + [架构蓝图 §8 原型同步轨道](docs/架构蓝图.md)。
...
tn-ui      GPUI 前端(唯一链接 gpui 的库):style(共享 Calm Glass 令牌 + col/cola/soft_shadow/shadowed/icon,
           ...
           explorer(文件树侧栏)· viewer(文件/Diff 查看器 → 重设计为 quick_look 速览浮层,待端口)· workspace(标题栏 + 标签/n-ary 分屏 + 侧栏 +
