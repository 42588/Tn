...
- **WSL + 远程 Linux(SSH/SFTP)** — 已有 WSL、SSH 登录、远端文件树、远端预览/编辑和远端 git 改动流首版;真实远端端到端回归仍按已知问题跟踪。
...
- `config.toml` — `[general]` / `[font]` / `[appearance]` / `[quick_terminal]` / `[editor]` + `[[profiles]]` / `[[agents]]` / `[[keybindings]]`,字段全可省略(缺省回退内置默认)。
...
tn-pty     PTY 后端:ConPTY、WSL、SSH、SFTP 远端文件和远端命令               — headless
tn-config  配置 + 主题 + Quick Terminal、editor、profiles、agents schema  — headless
tn-shell   shell 集成:OSC 133/633/7 旁路解析 → BlockEvent + 集成脚本       — headless
tn-blocks  Warp 式命令块状态机(命令/输出/退出码/时长)                     — headless
tn-agent   Agent Host 平台:开放 agent 身份、capability、adapter 和 sidecar  — headless
tn-ai      Claude/Codex 本地会话用量解析 + 内置 adapter                     — headless
...
- 中文轻量入口:[README.zh-CN.md](README.zh-CN.md)。
...
- [docs/架构/工作区与代码包边界.md](docs/架构/工作区与代码包边界.md) — **crate 详情**:完整 workspace、依赖方向和新增 crate 规则。
- [docs/外部参考资料索引.md](docs/外部参考资料索引.md) — 从 Windows Terminal 与 Ghostty 源码提炼、映射到 Tn 的设计要点。
- [docs/共享记忆索引.md](docs/共享记忆索引.md) — **共享记忆**:Claude / Codex 共用规则、工作区约定、踩坑和后续提示。
- [docs/文档治理索引.md](docs/文档治理索引.md) — **文档治理**:母索引、子文档、中文命名和新增信息落点规则。
